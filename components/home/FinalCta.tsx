...
export function FinalCta({ onScan }: { onScan: () => void }) {
  ...
  return (
    <section className="px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24">
      <motion.div initial={reduced ? { opacity: 0 } : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: reduced ? 0 : 0.6 }} className="max-w-5xl mx-auto rounded-3xl bg-gradient-to-r from-primary-600 to-primary-500 px-6 sm:px-12 py-10 sm:py-14 text-center shadow-xl">
