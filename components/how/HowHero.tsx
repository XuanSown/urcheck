...
export function HowHero() {
  ...
  const titleParts = t('how_title').split(' ');
          ...
          {titleParts.map((word, i) => (
            <motion.span key={i} variants={reveal} className="inline-block mr-[0.25em]">
              {i === titleParts.length - 1 ? (
                <span className="bg-gradient-to-r from-primary-600 via-primary-500 to-primary-400 bg-clip-text text-transparent dark:from-primary-400 dark:via-primary-300 dark:to-primary-200">
                  {word}
                </span>
              ) : (
                word
