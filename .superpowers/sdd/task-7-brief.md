...
**File to create:** `app/routines/[shareToken]/page.tsx`

Code verbatim từ plan:
...
## Global Constraints
- Trang CÔNG KHAI (không auth gate trước khi xem; chỉ clone yêu cầu login).
- Dùng `GET /api/routines/[shareToken]` (Task 2 enrich). 
- `handleClone`: nếu !customer → redirect `/customer/login?next=/routines/${shareToken}`; nếu customer → POST `/api/customer/routines` clone items.
- Dùng `TIME_ORDER`/`groupItemsByTimeOfDay` từ `lib/routine-utils` (Task 6 đã tạo).
- Verify: `npx tsc --noEmit` + `npm run build` pass.

## Commit
`git add app/routines/[shareToken]/page.tsx && git commit -m "feat: trang xem shared routine + clone về tài khoản"`

## Report: `.superpowers/sdd/task-7-report.md`. Trả status + commits + 1 dòng summary.
